...
class EESelectionReportGenerator(ReportGenerator):
    ...
    def generate(self, person: str, filePath: str) -> bool:
        ...
        personalSheet = book.add_sheet('個人加總成績', cell_overwrite_ok=True)
        # Write personal sheet column name
        personalSheet.write(0, 0, '姓名')
        personalSheet.write(0, 1, '分數')

        # Write personal sheet content
        personalSheet.write(1, 0, person)
        personalSheet.write(1, 1, str(individualScore))

        groupSheet = book.add_sheet('加成成績列表', cell_overwrite_ok=True)
        # Write group sheet column name
        groupSheet.write(0, 0, '加成項目')
        groupSheet.write(0, 1, '分數')
